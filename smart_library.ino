...
const char* WS_HOST  = "caboose.proxy.rlwy.net";
const uint16_t WS_PORT = 16869;
...
void onWSEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    ...
    case WStype_CONNECTED:
      ...
      // Send handshake immediately so server registers us as ESP32
      {
    ...
    case WStype_ERROR:
      Serial.printf("[WS] Error (code=%u): %s\n", (unsigned)length, (char*)payload);
      break;
...
void setup() {
  ...
  // WebSocket - Sattler library
  // IMPORTANT: setExtraHeaders must come before beginSSL


  // beginSSL(host, port, path, fingerprint)
  // Empty fingerprint string = accept any certificate (no pinning)
